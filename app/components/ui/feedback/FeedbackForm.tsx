...
function FeedbackForm() {
    ...
    function sendFeedback() {
        const { name: nameValue, raiting: raitingValue, message: messageValue } = formState;
        ...
        if (!hasErrors) {
            console.log("Form results:", formState);
            setSendingStatus({
                isVissible: true,
                status: true
            })
                ...
                >
                    <form className="relative z-10 bg-white p-4 md:p-12 rounded-xl shadow-sm max-w-[95%] md:max-w-md w-full">
                        ...
                        <div
                            ...
                            onMouseLeave={() => setHoveredStar(formState.raiting)}
                        >
                            {[1, 2, 3, 4, 5].map((ratingValue) => (
                                <img
                                    ...
                                    onClick={() => {
                                        setFormState({ ...formState, raiting: ratingValue })
                                        setErrors({ ...errors, raiting: false })
                                    }}
                                />
                            ))}
                        </div>
                        ...
                        <div className="relative">
                            ...
                            <textarea
                                id="messageText"
                                className={`${openSansFont.className} ${errors.message ? "ring-1 ring-t-red" : ""} w-full h-24 mt-2 bg-gray-200 text-gray-600 p-3 rounded-lg focus:outline-blue-600 placeholder-gray-400 mb-4 resize-none`} placeholder="Допоможіть нам покращити платформу."
                            ...
                            ></textarea>
                            {/* ERRORS MESSAGE */}
                            {(errors.message || errors.name || errors.raiting) && <p className={errorLabelStyles}><WarningIcon /> Заповніть усі поля та оберіть рейтинг</p>}
                        </div>
